...
def on_message(client, userdata, msg):
    ...
    if msg.topic == '/topic/binary':
        ...
        print(
            f'Receiving binary from esp and comparing with {binary}, size {bin_size}...'
        )
        ...
        with open(binary, 'rb') as f:
            ...
            if bin[:bin_size] == msg.payload[:bin_size]:
                ...
                return
            recv_binary = f'{binary}.received'
            with open(recv_binary, 'w') as fw:
                fw.write(msg.payload)
            raise ValueError(
                f'Received binary (saved as: {recv_binary}) does not match the original file: {binary}'
            )
    ...
    message_log += f'Received data:{msg.topic} {payload}' + '\n'
...
def test_examples_protocol_mqtt_ssl(env, extra_data):
    ...
    ttfw_idf.log_performance('mqtt_ssl_bin_size', f'{bin_size // 1024}KB')
    # Look for host:port in sdkconfig
    try:
        value = re.search(r'\:\/\/([^:]+)\:([0-9]+)', dut1.app.get_sdkconfig()['CONFIG_BROKER_URI'])
        broker_url = value[1]
        broker_port = int(value[2])
    ...
    try:
        ...
        print(
            f'ENV_TEST_FAILURE: Unexpected error while connecting to broker {broker_url}: {sys.exc_info()[0]}:'
        )
    ...
    try:
        ...
        if not event_client_connected.wait(timeout=30):
            raise ValueError(
                f'ENV_TEST_FAILURE: Test script cannot connect to broker: {broker_url}'
            )
